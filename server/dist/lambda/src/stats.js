import { dayKeyLocal, effectiveTs } from "./merge.js";
/**
 ...
 */
export function buildStats(sessions, wallHits, now = Date.now()) {
    const byEmail = new Map();
    const byModel = new Map();
    const accModelStat = (model, u) => {
    ...
    for (const s of sessions) {
        if (s.by_model && typeof s.by_model === "object") {
            for (const [model, u] of Object.entries(s.by_model))
                accModelStat(model, u);
        }
        else if (s.model) {
    ...
    for (const wh of wallHits) {
        ...
        if (!agg)
            continue;
        if (dayKeyLocal(wh.at) === todayLocal)
            agg.hit_wall_today = true;
        if (wh.at >= now - SEVEN_DAYS)
            agg.hit_wall_7d = true;
