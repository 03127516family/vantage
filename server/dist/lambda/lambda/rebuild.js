...
import { createMergeState, mergeInto } from "../src/merge.js";
...
export async function runRebuild(deps) {
    ...
    if (viewOk) {
        try {
            const wm = JSON.parse(viewRes.body).watermark;
    ...
    if (indexRes.status === 200) {
        for (const line of indexRes.body.split("\n")) {
            if (!line.trim())
                continue;
            try {
                mergeInto(state, JSON.parse(line));
            }
            catch {
                // 跳过损坏行
            }
    ...
    if (wallRes.status === 200) {
        try {
            for (const wh of JSON.parse(wallRes.body)) {
    ...
    if (listRes.status !== 200)
        throw new Error(`LIST events 失败 status=${listRes.status}`);
    ...
    for (let i = 0; i < newKeys.length; i += GET_CONCURRENCY) {
        ...
        for (const r of batch) {
            if (r.status !== 200)
                throw new Error(`GET event 失败 status=${r.status}`);
            bodies.push(r.body);
        }
    }
    let skipped = 0;
    for (const body of bodies) {
        try {
            mergeInto(state, JSON.parse(body));
        }
        catch {
            skipped += 1;
        }
    ...
    if (newKeys.length > 0 || !viewOk) {
        ...
        if (!ok2xx(p1.status))
            throw new Error(`PUT index.jsonl 失败 status=${p1.status}`);
        const p2 = await deps.put(`${p}state/wallhits.json`, JSON.stringify(state.wallHits));
        if (!ok2xx(p2.status))
            throw new Error(`PUT wallhits.json 失败 status=${p2.status}`);
        const p3 = await deps.put(`${p}state/stats-view.json`, JSON.stringify(view));
        if (!ok2xx(p3.status))
            throw new Error(`PUT stats-view.json 失败 status=${p3.status}`);
