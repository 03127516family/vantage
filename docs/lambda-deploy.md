...
4. 阶段:从 Lambda 触发器创建时默认给的是**命名阶段 `default`**(URL 形如 `.../default/`)。handler 已自动剥离阶段前缀,`$default` 与任意命名阶段均可直接用;员工端 `server_url` 带上完整阶段路径即可(如 `https://<api-id>.execute-api.cn-north-1.amazonaws.com.cn/default`)
